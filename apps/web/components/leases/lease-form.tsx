...
import { createLeaseAction, type LeaseState } from "@/lib/actions/leases";
...
export function LeaseForm({
  ...
  const action = createLeaseAction.bind(null, locale);
      ...
      <section>
        ...
        <div className="grid grid-cols-1 gap-5 sm:grid-cols-2">
          ...
          <Field label={t("fields.endDate")} htmlFor="end_date">
            <input id="end_date" name="end_date" type="date" required className={fieldClass} />
          </Field>
          <Field label={t("fields.rent")} htmlFor="rent_amount">
            <input id="rent_amount" name="rent_amount" type="number" inputMode="decimal" step="0.01" min="0" required dir="ltr" className={`${fieldClass} text-start`} />
          </Field>
          <Field label={t("fields.frequency")} htmlFor="payment_frequency">
            <select id="payment_frequency" name="payment_frequency" defaultValue="monthly" className={fieldClass}>
              {frequencies.map((f) => (
          ...
          <Field label={t("fields.deposit")} htmlFor="deposit_amount" hint={tc("optional")}>
            <input id="deposit_amount" name="deposit_amount" type="number" inputMode="decimal" step="0.01" min="0" dir="ltr" className={`${fieldClass} text-start`} />
          </Field>
          <Field label={t("fields.contractNumber")} htmlFor="contract_number" hint={tc("optional")}>
            <input id="contract_number" name="contract_number" dir="ltr" className={`${fieldClass} text-start`} />
          </Field>
        </div>
      </section>
      ...
      <section>
        ...
        <div className="grid grid-cols-1 gap-5 sm:grid-cols-3">
          <Field label={t("fields.lateFeeType")} htmlFor="late_fee_type">
            <select id="late_fee_type" name="late_fee_type" defaultValue="none" className={fieldClass}>
              {lateFeeTypes.map((lt) => (
          ...
          <Field label={t("fields.lateFeeValue")} htmlFor="late_fee_value" hint={tc("optional")}>
            <input id="late_fee_value" name="late_fee_value" type="number" inputMode="decimal" step="0.01" min="0" dir="ltr" className={`${fieldClass} text-start`} />
          </Field>
          <Field label={t("fields.grace")} htmlFor="grace_period_days" hint={tc("optional")}>
            <input id="grace_period_days" name="grace_period_days" type="number" inputMode="numeric" min="0" dir="ltr" className={`${fieldClass} text-start`} />
          </Field>
        </div>
        <div className="mt-5 flex flex-wrap items-center gap-x-8 gap-y-3">
          ...
          <label className="flex items-center gap-2.5 self-end pb-2.5 text-sm font-medium text-brand-teal-900">
            <input type="checkbox" name="auto_renew" className="h-4 w-4 rounded border-brand-teal/30 text-brand-teal focus:ring-brand-teal/30" />
            {t("fields.autoRenew")}
          </label>
        </div>
      </section>
